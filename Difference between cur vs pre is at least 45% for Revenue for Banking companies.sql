...
SET @frDate = '2020-09-01 00:00:00.000'
...
where ed.versionid > 111 and ed.companyId >11 and EDND.dataItemId = 21642
...
select ED.versionId,ED.companyId,ed.tradingitemid,ED.parentFlag,ED.accountingStandardId,ed.effectiveDate,ed.researchContributorId,
EDND.dataItemId,DM.dataitemName as PreDataitemname,dbo.formatPeriodId_fn(ed.estimatePeriodId)as peo,ed.estimatePeriodId,ednd.dataitemvalue as Prevalue,
...
where ed.versionid > 111 and ed.companyId >11 and EDND.dataItemId in (21642)

---#Pdocs--previous data ----#guid1--- current

IF OBJECT_ID('TEMPDB..#final') IS NOT NULL DROP TABLE #final
select ED.versionId,ED.companyId,ed.tradingitemid,ED.parentFlag,ED.accountingStandardId,ed.effectiveDate,ed.researchContributorId,
ed.dataItemId,ed.CurrDataitemname,apeo,ed.dataitemvalue currvalue, ed.currencyId,ed.unitsId,pds.versionId as PreVID,pds.dataItemId as PreDataItemId, PreDataitemname,pds.peo,
pds.Prevalue
...
Select distinct ED.*,
CASE 
WHEN (currvalue ) != 0 and abs(currvalue)>abs (Prevalue) THEN (ABS(((currvalue) - (Prevalue))/(Prevalue)) * 100)  
WHEN (Prevalue ) != 0 and abs(currvalue)<=abs(Prevalue) THEN (ABS(((currvalue) - (Prevalue))/(currvalue)) * 100)
WHEN ed.unitsId != 2 THEN ed.unitsId = 2 and pds.unitsId = 2
WHEN pds.unitsId != 2 THEN pds.unitsId = 2 and ed.unitsId = 2
END AS [DiffPer] into #final1
FROM #final ed

select * from #final1 where DiffPer >= 45
