...
SELECT DISTINCT CT.*,B.Cat,P.priorityName,CONVERT(VARCHAR(10),ct.Enddate-'6:00',101)Date,CONVERT(VARCHAR(10),insertedDate-'6:00',101)Date2,cp.collectionProcessName INTO #CT 
...
WHERE ct.collectionStageId in (2,122)  AND CT.collectionProcessId IN (64,1062,4861,4862,4863,4864,4899,4900,4901,4902,4903,4904,4905,4906,4907) AND collectionEntityTypeId = 9
...
INSERT INTO #CT 
SELECT DISTINCT CT.*,B.Cat,P.priorityName,Convert(varchar(10),ct.Enddate-'6:00',101)Date,Convert(varchar(10),insertedDate-'6:00',101)Date2,cp.collectionProcessName 
FROM WorkflowArchive_Estimates.dbo.CommonTracker_vw CT
...
FROM #CT WHERE Enddate> = @frDate  AND Enddate <= @toDate AND collectionStageStatusId IN (4) --considered 4 for processed records---2:In Process,3:Pending,34:Issued to Checks Review
GROUP BY Cat,Date
