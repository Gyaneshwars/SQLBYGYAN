...
SET @frDate = '2023-04-01 00:00:00.000'
SET @toDate = '2023-06-30 23:59:59.999'
...
IF OBJECT_ID('TEMPDB..#CTFlow') IS NOT NULL DROP TABLE #CTFlow
SELECT CT.*,ISNULL(B.Cat,C.Cat) AS Cat,P.priorityName,cp.collectionProcessName,CONVERT(date,CT.insertedDate) AS Date2,CONVERT(date,CT.endDate) AS Date INTO #CTFlow FROM WorkflowArchive_Estimates.dbo.CommonTracker_vw CT
LEFT JOIN WorkflowArchive_Estimates.dbo.CollectionEntityToProcessToDataPoint_tbl cdp (NOLOCK) ON cdp.collectionEntityToProcessId = ct.collectionEntityToProcessId and datapointid  = 1887
...
LEFT JOIN #CatFS C (NOLOCK) ON CT.collectionStageId = C.collectionStageId and ct.PriorityID = b.PriorityID
WHERE CT.collectionStageId IN (2,122,155,215,216,219,220,221)  AND CT.collectionProcessId IN (64,1062,4861,4862,4863,4864,4899,4900,4901,4902,4903,4904,4905,4906,4907,4897,4898) AND collectionEntityTypeId = 9
...
FROM #CTFlow WHERE Enddate> = @frDate  AND Enddate <= @toDate AND collectionStageStatusId IN (4) --considered 4 for processed records---2:In Process,3:Pending,34:Issued to Checks Review
GROUP BY Cat,Date
--select * from #Processed

IF OBJECT_ID('TEMPDB..#Inserted') IS NOT NULL DROP TABLE #Inserted
SELECT DISTINCT Date2 Date,Cat,COUNT(collectionEntityId)Cnt INTO #Inserted FROM #CTFlow
...
SELECT DISTINCT Date,Cat,COUNT(collectionEntityId)Cnt INTO #Skipp FROM #CTFlow 
...
SELECT DISTINCT Date2 Date,Cat,COUNT(collectionEntityId)Cnt INTO #InpPIcr FROM #CTFlow 
WHERE insertedDate> = @frDate  AND insertedDate <= @toDate AND collectionStageStatusId IN (2,3,34)
GROUP BY Cat,Date2
...
SELECT DISTINCT Date2 Date,Cat,collectionStageId,collectionProcessName, COUNT(collectionEntityId)Cnt FROM #CTFlow
