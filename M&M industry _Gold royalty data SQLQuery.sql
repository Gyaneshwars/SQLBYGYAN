...
IF OBJECT_ID('tempdb..#MMI') IS NOT NULL DROP TABLE #MMI
...
INTO #MMI FROM Estimates.[dbo].EstimateDetail_tbl ed (NOLOCK)
...
WHERE cs.businessDescription  LIKE '%royalty company%'
AND ed.effectiveDate>=@frdate AND ed.effectiveDate<=@todate
AND edn.dataItemId NOT IN (600265,600318)
--AND ed.companyId IN (673583274)
ORDER BY ed.effectiveDate



SELECT DISTINCT versionId,companyId,companyName,Contributor,FilingDate,businessDescription FROM #MMI ORDER BY FilingDate

--SELECT DISTINCT * FROM #MMI ORDER BY FilingDate


--SELECT DISTINCT companyId,Contributor FROM #MMI
